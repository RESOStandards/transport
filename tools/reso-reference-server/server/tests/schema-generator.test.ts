...
describe('edmTypeToSql', () => {
  ...
  it('maps Edm.Decimal with precision/scale', () => {
    expect(edmTypeToSql(makeField({ fieldName: 'Price', type: 'Edm.Decimal', precision: 14, scale: 2 }))).toBe('NUMERIC(14,2)');
  });
  ...
  it('maps Edm.DateTimeOffset to TIMESTAMPTZ', () => {
    expect(edmTypeToSql(makeField({ fieldName: 'ModTime', type: 'Edm.DateTimeOffset' }))).toBe('TIMESTAMPTZ');
  });

  it('maps enum types to TEXT', () => {
    expect(edmTypeToSql(makeField({ fieldName: 'Status', type: 'org.reso.metadata.enums.StandardStatus' }))).toBe('TEXT');
  });

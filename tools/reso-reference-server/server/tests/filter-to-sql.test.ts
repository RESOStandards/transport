...
describe("filterToSql", () => {
  ...
  describe("string functions", () => {
    ...
    it("translates concat", () => {
      const result = filterToSql(
        "concat(City, StateOrProvince) eq 'AustinTX'",
        fields,
        "p",
      );
      expect(result.where).toBe(
        `(p."City" || p."StateOrProvince") = $1`,
      );
  ...
  describe("date functions", () => {
    it("translates year", () => {
      ...
      expect(result.where).toBe(
        `EXTRACT(YEAR FROM p."ModificationTimestamp") = $1`,
      );
    ...
    it("translates month", () => {
      const result = filterToSql(
        "month(ModificationTimestamp) eq 6",
        fields,
        "p",
      );
      expect(result.where).toBe(
        `EXTRACT(MONTH FROM p."ModificationTimestamp") = $1`,
      );
  ...
  describe("complex expressions", () => {
    it("handles nested logical operators", () => {
      const result = filterToSql(
        "ListPrice gt 200000 and (City eq 'Austin' or City eq 'Dallas')",
        fields,
        "p",
      );
      expect(result.where).toBe(
        `(p."ListPrice" > $1 AND (p."City" = $2 OR p."City" = $3))`,
      );
    ...
    it("handles function in comparison", () => {
      const result = filterToSql(
        "contains(City, 'Aus') and ListPrice gt 100000",
        fields,
        "p",
      );
      expect(result.where).toBe(
        `(p."City" ILIKE $1 AND p."ListPrice" > $2)`,
      );
  ...
  describe("error handling", () => {
    it("throws on unknown field", () => {
      expect(() =>
        filterToSql("UnknownField eq 'test'", fields, "p"),
      ).toThrow("Unknown field in $filter: UnknownField");
    });
  });
});

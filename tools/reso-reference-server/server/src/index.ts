...
import { loadMetadata, getFieldsForResource, getKeyFieldForResource } from "./metadata/loader.js";
...
const main = async (): Promise<void> => {
  ...
  console.log(
    `Loaded RESO metadata v${metadata.version}: ${metadata.fields.length} fields, ${metadata.lookups.length} lookups`,
  );

  // Create database pool and run migrations
  const pool = createPool(config.databaseUrl);

  const resourceSpecs = TARGET_RESOURCES
    .map((resource) => ({

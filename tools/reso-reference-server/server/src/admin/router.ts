 ...
 */
export const createAdminRouter = (metadata: ResoMetadata, dal: DataAccessLayer, authConfig: AuthTokenConfig): Router => {
  const router = Router();
  const adminAuth = requireAuth('admin', authConfig);

  // Data generator endpoints
  router.post('/admin/data-generator', adminAuth, createDataGeneratorHandler(metadata, dal));

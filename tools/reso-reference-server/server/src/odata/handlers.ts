...
const prefersMinimal = (prefer: string | undefined): boolean =>
  prefer?.includes("return=minimal") ?? false;
...
export const createHandler = (ctx: HandlerContext): RequestHandler =>
  async (req, res) => {
    try {
...
export const readHandler = (ctx: HandlerContext): RequestHandler =>
  async (req, res) => {
    try {
      const key = extractKey(req.path);
      if (!key) {
        setODataHeaders(res);
        res.status(400).json(buildValidationError([
          { field: "key", reason: "Missing resource key in URL" },
        ]));
        return;
      }
...
export const updateHandler = (ctx: HandlerContext): RequestHandler =>
  async (req, res) => {
    try {
      const key = extractKey(req.path);
      if (!key) {
...
export const deleteHandler = (ctx: HandlerContext): RequestHandler =>
  async (req, res) => {
    try {
      const key = extractKey(req.path);
      if (!key) {
...
export const collectionHandler = (ctx: HandlerContext): RequestHandler =>
  async (req, res) => {
    try {

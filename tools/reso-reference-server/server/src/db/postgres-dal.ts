...
const parseSelect = (select: string): ReadonlyArray<string> =>
  select.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
...
const buildSelectColumns = (
  ...
  if (select) {
    ...
    selectedNames = fields
      .filter((f) => requested.has(f.fieldName))
      .map((f) => f.fieldName);
  } else {
 ...
 */
const buildNavSelectColumns = (
  binding: NavigationPropertyBinding,
  alias: string,
): ReadonlyArray<string> =>
  binding.targetFields.map(
    (f) => `${alias}."${f.fieldName}" AS "${alias}.${f.fieldName}"`,
  );
...
const buildExpandJoin = (
  ...
  return (
    `LEFT JOIN "${binding.targetResource}" ${navAlias} ` +
    `ON ${navAlias}."${targetCol}" = ${parentAlias}."${parentKeyField}"`
  );
};
...
export const createPostgresDal = (pool: pg.Pool): DataAccessLayer => {
  const queryCollection = async (
    ...
    const selectCols: string[] = [
      ...buildSelectColumns(ctx.fields, ctx.keyField, options?.$select, parentAlias),
    ];
    ...
    if (options?.$expand) {
      ...
      const bindingMap = new Map(
        ctx.navigationBindings.map((b) => [b.name, b]),
      );
    ...
    let whereClause = "";
    if (options?.$filter) {
      const filterResult = filterToSql(
        options.$filter,
        ctx.fields,
        parentAlias,
        paramIndex,
    ...
    const countCol = options?.$count
      ? `, COUNT(*) OVER() AS "__total_count"`
      : "";

    // Assemble final SQL
    const sql = [
      `SELECT ${selectCols.join(", ")}${countCol}`,
      `FROM ${fromClause}`,
      whereClause,
      orderByClause,
      limitClause,
  ...
  const readByKey = async (
    ...
    const parent = extractParentColumns(
      row,
      parentAlias,
      ctx.fields,
    );
  ...
  const update = async (
    ...
    const entries = Object.entries(updates).filter(
      ([key]) => key !== ctx.keyField && fieldMap.has(key),
    );

...
import type { ArithmeticOperator, ComparisonOperator, FilterExpression, FilterFunctionName, Token } from './types.js';
...
const peek = (state: ParserState): Token | undefined => (state.pos < state.tokens.length ? state.tokens[state.pos] : undefined);

const advance = (state: ParserState): Token => {
  const token = state.tokens[state.pos];
  if (!token) {
    const lastPos = state.tokens.length > 0 ? state.tokens[state.tokens.length - 1].position : 0;
...
const expect = (state: ParserState, type: string, value?: string): Token => {
  ...
  if (token.type !== type || (value !== undefined && token.value !== value)) {
    throw new ParseError(`Expected ${value ? `'${value}'` : type} but got '${token.value}'`, token.position);
  }
  return token;
};
...
const parsePrimaryExpr = (state: ParserState): FilterExpression => {
  ...
  switch (token.type) {
    ...
    case 'literal_number': {
      advance(state);
      const numValue = token.value.includes('.') ? Number.parseFloat(token.value) : Number.parseInt(token.value, 10);
    ...
    case 'property': {
      ...
      while (peek(state)?.type === 'slash') {
        ...
        if (next?.type === 'lambda_op') {
          ...
          throw new ParseError(`Expected property name or lambda operator after '/'`, next?.position ?? 0);
        }
      }
...
const parseLambda = (state: ParserState, source: FilterExpression): FilterExpression => {
  ...
  if (varToken.type !== 'property') {
    throw new ParseError(`Expected lambda variable name but got '${varToken.value}'`, varToken.position);
  }
...
export const parseFilter = (filterString: string): FilterExpression => {
  ...
  if (state.pos < state.tokens.length) {
    ...
    throw new ParseError(`Unexpected token '${remaining.value}' after end of expression`, remaining.position);
  }

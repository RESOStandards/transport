...
const generateEtag = (): string => `W/"${Buffer.from(new Date().toISOString()).toString('base64')}"`;
...
const findNegativeNumericField = (body: Record<string, unknown>): [string, unknown] | undefined =>
  Object.entries(body).find(([, v]) => typeof v === 'number' && v < 0);

/** Constructs an OData-compliant error response body with error.code, error.message, and error.details. */
const makeErrorResponse = (target: string, fieldName: string, message: string): Record<string, unknown> => ({
  error: {
    code: '20100',
...
export const handleCreate =
  ...
  (req: Request, res: Response): void => {
    ...
    if (negativeField) {
      ...
      res.status(400).json(makeErrorResponse('Create', negativeField[0], `${negativeField[0]} must be greater than or equal to 0`));
      return;
    }
...
export const handleUpdate =
  ...
  (req: Request, res: Response): void => {
    ...
    if (!key) {
      res.set('OData-Version', '4.01');
      res.status(400).json(makeErrorResponse('Update', 'key', 'Missing resource key in URL'));
      return;
    }
    ...
    if (negativeField) {
      res.set('OData-Version', '4.01');
      res.status(400).json(makeErrorResponse('Update', negativeField[0], `${negativeField[0]} must be greater than or equal to 0`));
      return;
    }

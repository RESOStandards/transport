...
import type { ODataResponse, EntityType } from "../src/lib/types.js";

const makeResponse = (
  overrides: Partial<ODataResponse> = {},
): ODataResponse => ({
  status: 200,
...
describe("validateStatusCode", () => {
  it("passes when status is in allowed list", () => {
    const result = validateStatusCode(makeResponse({ status: 201 }), [
      201, 204,
    ]);
  ...
  it("fails when status is not in allowed list", () => {
    const result = validateStatusCode(makeResponse({ status: 500 }), [
      201, 204,
    ]);
...
describe("validateODataVersionHeader", () => {
  it("passes for 4.01", () => {
    const result = validateODataVersionHeader(
      makeResponse({ headers: { "odata-version": "4.01" } }),
    );
    expect(result.status).toBe("pass");
  });
  ...
  it("passes for 4.0", () => {
    const result = validateODataVersionHeader(
      makeResponse({ headers: { "odata-version": "4.0" } }),
    );
    expect(result.status).toBe("pass");
  });
...
describe("validateEntityIdHeader", () => {
  ...
  it("passes for 204 with EntityId", () => {
    const result = validateEntityIdHeader(
      makeResponse({ status: 204, headers: { entityid: "123" } }),
    );
    expect(result.status).toBe("pass");
  });
  ...
  it("fails for 204 without EntityId", () => {
    const result = validateEntityIdHeader(
      makeResponse({ status: 204, headers: {} }),
    );
    expect(result.status).toBe("fail");
  });
});
...
describe("validateLocationHeader", () => {
  ...
  it("fails when Location is not a valid URL", () => {
    const results = validateLocationHeader(
      makeResponse({ headers: { location: "not-a-url" } }),
      "Property",
    );
...
describe("validateEmptyResponse", () => {
  ...
  it("fails for non-empty body", () => {
    const result = validateEmptyResponse(
      makeResponse({ rawBody: '{"foo":"bar"}' }),
    );
    expect(result.status).toBe("fail");
  });
});
...
describe("validateODataAnnotation", () => {
  ...
  it("fails when etag does not start with W/", () => {
    ...
    const etagCheck = results.find((r) =>
      r.description.includes("starts with"),
    );
...
describe("validateResponseContainsPayload", () => {
  ...
  it("fails when a payload field is missing", () => {
    ...
    const bedroomCheck = results.find((r) =>
      r.description.includes("BedroomsTotal"),
    );
...
describe("validateODataError", () => {
  ...
  it("fails when details target is not in metadata", () => {
    ...
    const targetCheck = results.find((r) =>
      r.description.includes("UnknownField"),
    );

...
describe('Full scenario run against mock server', () => {
  it('all 8 scenarios pass against a compliant mock', async () => {
    ...
    for (const scenario of report.scenarios) {
      const failedAssertions = scenario.assertions.filter(a => a.status === 'fail');
      if (failedAssertions.length > 0) {
        const details = failedAssertions.map(a => `  - ${a.description}: expected=${a.expected}, actual=${a.actual}`).join('\n');
        expect.fail(`Scenario "${scenario.scenario}" failed:\n${details}`);
      }
    }

...
import type { ODataResponse, ODataErrorBody, ODataErrorDetail } from "../types.js";
...
export const getErrorTargets = (
  ...
  return details
    .filter((d): d is ODataErrorDetail & { target: string } => typeof d.target === "string")
    .map((d) => d.target);
};

...
const parseProperties = (
  ...
  rawProperties.map((rawProp) => {
    ...
    const rawAnnotations =
      (rawProp["Annotation"] as ReadonlyArray<Record<string, string>> | undefined) ?? [];
 ...
 */
const parseReferentialConstraints = (
  rawConstraints: ReadonlyArray<Record<string, unknown>>,
): ReadonlyArray<CsdlReferentialConstraint> =>
...
const parseNavigationProperties = (
  ...
  rawNavProps.map((rawNav) => {
    ...
    const rawConstraints =
      (rawNav["ReferentialConstraint"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
...
const parseEntityTypes = (
  ...
  rawEntityTypes.map((rawEntity) => {
    ...
    const rawNavProperties =
      (rawEntity["NavigationProperty"] as ReadonlyArray<Record<string, unknown>>) ?? [];
...
const parseComplexTypes = (
  ...
  rawComplexTypes.map((rawComplex) => {
    ...
    const rawProperties =
      (rawComplex["Property"] as ReadonlyArray<Record<string, unknown>>) ?? [];
    const rawNavProperties =
      (rawComplex["NavigationProperty"] as ReadonlyArray<Record<string, unknown>>) ?? [];
...
const parseEnumTypes = (
  rawEnumTypes: ReadonlyArray<Record<string, unknown>>,
): ReadonlyArray<CsdlEnumType> =>
  rawEnumTypes.map((rawEnum) => {
    const name = rawEnum["@_Name"] as string;
    const rawMembers =
      (rawEnum["Member"] as ReadonlyArray<Record<string, string>> | undefined) ?? [];
...
const parseActions = (
  ...
  rawActions.map((rawAction) => {
    const rawParams =
      (rawAction["Parameter"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
...
const parseFunctions = (
  ...
  rawFunctions.map((rawFunc) => {
    const rawParams =
      (rawFunc["Parameter"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
...
const parseEntityContainer = (
  ...
  // Entity sets
  const rawEntitySets =
    (rawContainer["EntitySet"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
  const entitySets: ReadonlyArray<CsdlEntitySet> = rawEntitySets.map((es) => {
    const rawBindings =
      (es["NavigationPropertyBinding"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
    return {
  ...
  // Singletons
  const rawSingletons =
    (rawContainer["Singleton"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
  const singletons: ReadonlyArray<CsdlSingleton> = rawSingletons.map((s) => {
    const rawBindings =
      (s["NavigationPropertyBinding"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
    return {
  ...
  // Action imports
  const rawActionImports =
    (rawContainer["ActionImport"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
  ...
  // Function imports
  const rawFunctionImports =
    (rawContainer["FunctionImport"] as ReadonlyArray<Record<string, unknown>> | undefined) ?? [];
...
export const parseCsdlXml = (xml: string): CsdlSchema => {
  ...
  const rawEntityTypes: ReadonlyArray<Record<string, unknown>> =
    schema["EntityType"] ?? [];
  const rawEnumTypes: ReadonlyArray<Record<string, unknown>> =
    schema["EnumType"] ?? [];
  const rawComplexTypes: ReadonlyArray<Record<string, unknown>> =
    schema["ComplexType"] ?? [];
  const rawActions: ReadonlyArray<Record<string, unknown>> =
    schema["Action"] ?? [];
  const rawFunctions: ReadonlyArray<Record<string, unknown>> =
    schema["Function"] ?? [];
  const rawContainer = schema["EntityContainer"] as
...
export const getEnumType = (
  schema: CsdlSchema,
  name: string,
): CsdlEnumType | undefined =>
  schema.enumTypes.find((et) => et.name === name);

...
const main = async (): Promise<void> => {
  ...
  if (schema.entityContainer) {
    console.log(
      `Entity sets: ${schema.entityContainer.entitySets.length}`,
    );
  }
  ...
  for (const et of schema.entityTypes) {
    console.log(
      `  ${et.name} — ${et.properties.length} properties, key: [${et.key.join(", ")}]`,
    );
  }
  ...
  if (property) {
    ...
    for (const prop of property.properties.slice(0, 10)) {
      console.log(
        `  ${prop.name}: ${prop.type}${prop.nullable === false ? " (required)" : ""}`,
      );
    }

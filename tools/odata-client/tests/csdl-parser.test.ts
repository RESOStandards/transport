...
import { parseCsdlXml, getEntityType, getEnumType } from "../src/csdl/parser.js";
...
describe("parseCsdlXml", () => {
  ...
  it("throws on missing Schema element", () => {
    expect(() => parseCsdlXml("<foo/>")).toThrow(
      "Could not find Schema element",
    );
  });
});
...
describe("parseCsdlXml — sample-metadata.xml compatibility", () => {
  it("parses the test tool's sample metadata", async () => {
    const samplePath = resolve(
      import.meta.dirname,
      "../../web-api-add-edit-test/sample-metadata.xml",
    );
    let xml: string;
    try {

...
- **Diff before commit.** Use [`reso-tools/reso-certification/utils/diff-dd-sheet.py`](https://github.com/RESOStandards/reso-tools/blob/main/reso-certification/utils/diff-dd-sheet.py) to surface the row-level deltas between the prior and proposed XLSX. The tool emits markdown suitable for both ticket comments and `CHANGELOG.md` entries (`--changelog` flag).
- **CHANGELOG.md entry on every revision.** Each XLSX change gets a row-level breakdown in `references/dd/CHANGELOG.md`, linked to the originating transport ticket. Silent edits without a changelog entry mislead downstream consumers about coverage.

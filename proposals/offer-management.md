...
**The organization identifier elements are optional and run in parallel. They do not replace anything until Data Dictionary 3.0.** `OfferUoi` and `OfferUsi` are added now so a provider can adopt them early, and they are carried in addition to the originating system pair, never instead of it. A provider implementing a new element must also support the current standard until the next major version ([RESO versioning](https://github.com/RESOStandards/transport/blob/main/versioning.md)), so a provider publishing `OfferUoi` and neither `OfferOriginatingSystemName` nor `OfferOriginatingSystemId` fails against the current standard.
...
The third is a question for the workgroups rather than a claim this proposal can make. [RESO versioning](https://github.com/RESOStandards/transport/blob/main/versioning.md) makes adding a duplicate or replacement data element a major change, because it forces users to map to multiple elements for the same thing. Two additions here could be read that way, and both are declared rather than left for review to find. In both cases the argument is the same: the element being added is new, on a new resource, so nothing already in the standard acquires a second spelling.

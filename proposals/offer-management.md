...
* **Is a minor change, targeted for the next minor Data Dictionary release.** Every element it reuses is used exactly as Data Dictionary 2.1 defines it, so no existing element changes, and its testing rules apply to new data elements, which RESO versioning makes minor. Two additions sit beside an existing element without replacing it: `OfferUoi` beside `OfferOriginatingSystemName` and `OfferOriginatingSystemId`, and `RequestedClosingDate` beside `CloseDate`. Neither forces a consumer to map two elements to one fact, which is what RESO versioning makes major. [Section 6](#section-6-appendices) sets out both. Which release carries this depends on how long it takes through the workgroups, which meet quarterly.
